...
if (window.soundManager) {
  soundManager.setup({ url: 'https://mraquamentus.github.io/GOBF/swf/' });
  if (window.Replays) soundManager.onready(window.Replays.soundReady);
  soundManager.onready(function () {
    soundManager.createSound({
      id: 'notif',
      url: 'https://mraquamentus.github.io/GOBF/audio/notification.wav' });
...
var Tools = {
  ...
  fxPrefix: function () {
    if (document.location.protocol === 'file:') {
      if (window.Replays) return 'https://mraquamentus.github.io/GOBF/fx/';
      return 'fx/';
    }
    return '//mraquamentus.github.io/GOBF/fx/';
  }(),
  ...
  parseChatMessage: function (message, name, timestamp, isHighlighted, $chatElem) {
    ...
    switch (cmd) {
      ...
      case 'data-pokemon':
        ...
        buf += '<span class="col pokemonnamecol" style="white-space:nowrap"><a href="https://mraquamentus.github.io/GOBF/dex/pokemon/' + template.id + '" target="_blank">' + template.species + '</a></span> ';
        ...
        var bst = 0;
        for (var _i in template.baseStats) {bst += template.baseStats[_i];}
      ...
      case 'uhtmlchange':
        ...
        var _html = parts.slice(1).join(',');
        if (!_html) {
          $elements.remove();
        } else if (!$elements.length) {
          $chatElem.append('<div class="chat uhtml-' + toId(parts[0]) + '">' + Tools.sanitizeHTML(_html) + '</div>');
        } else if (cmd === 'uhtmlchange') {
          $elements.html(Tools.sanitizeHTML(_html));
        } else {
          $elements.remove();
          $chatElem.append('<div class="chat uhtml-' + toId(parts[0]) + '">' + Tools.sanitizeHTML(_html) + '</div>');
        }
  ...
  sanitizeHTML: function () {
    ...
    var tagPolicy = function (tagName, attribs) {
      ...
      var dataUri = '';
      if (tagName === 'img') {
        for (var _i2 = 0; _i2 < attribs.length - 1; _i2 += 2) {
          if (attribs[_i2] === 'src' && attribs[_i2 + 1].substr(0, 11) === 'data:image/') {
            srcIdx = _i2;
            dataUri = attribs[_i2 + 1];
          }
          if (attribs[_i2] === 'src' && attribs[_i2 + 1].substr(0, 2) === '//') {
            if (location.protocol !== 'http:' && location.protocol !== 'https:') {
              attribs[_i2 + 1] = 'http:' + attribs[_i2 + 1];
            }
          }
        }
        ...
        var iconAttrib = null;
        for (var _i3 = 0; _i3 < attribs.length - 1; _i3 += 2) {
          if (attribs[_i3] === 'pokemon' || attribs[_i3] === 'item') {
            // If declared more than once, use the later.
            iconAttrib = attribs.slice(_i3, _i3 + 2);
          } else if (attribs[_i3] === 'class') {
            classValueIndex = _i3 + 1;
          } else if (attribs[_i3] === 'style') {
            styleValueIndex = _i3 + 1;
          }
        }
        tagName = 'span';
  ...
  getSpriteData: function (pokemon, siden) {var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : { gen: 6 };return function () {
  ...
  getPokemonIcon: function (pokemon, facingLeft) {
    ...
    if (pokemon && pokemon.num !== undefined) num = pokemon.num;else
  ...
  getTeambuilderSprite: function (pokemon) {var gen = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    ...
    if (gen <= 1 && template.gen <= 1) spriteDir = Tools.resourcePrefix + 'sprites/rby';else
    if (gen <= 2 && template.gen <= 2) spriteDir = Tools.resourcePrefix + 'sprites/gsc';else
    if (gen <= 3 && template.gen <= 3) spriteDir = Tools.resourcePrefix + 'sprites/rse';else
    if (gen <= 4 && template.gen <= 4) spriteDir = Tools.resourcePrefix + 'sprites/dpp';
...
var BattlePOStatNames = { // only used for interacting with PO
  hp: 'HP',
  atk: 'Atk',
  def: 'Def',
  spa: 'SAtk',
